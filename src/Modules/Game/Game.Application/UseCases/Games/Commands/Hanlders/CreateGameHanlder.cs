...
namespace Game.Application.UseCases.Games.Commands.Hanlders
{
    internal sealed class CreateGameHanlder : IRequestHandler<CreateGame, Guid>
    {
        ...
        public CreateGameHanlder(ILogger<CreateGameHanlder> logger,
            ...
            IRoomMemberRepository memberRepository, IGameTimerService timerService)
        {
            _logger = logger;
        ...
        public async Task<Guid> Handle(CreateGame command, CancellationToken cancellationToken)
        {
            ...
            var (_,gameType, hasTeams, hasGameStateTimer, roundQuantity, roomId) = command;
            ...
            await _notifications.GameCreatedForRoom(room.Id);
            await _notifications.GameCreatedForAll(room.Id);
